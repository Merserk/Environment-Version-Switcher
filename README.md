...
*   **Automatic Detection**: Scans multiple common locations to find all installed Python versions, including:
    *   User-specific installations (`%LOCALAPPDATA%\Programs\Python`)
    *   System-wide installations (`%ProgramFiles%`, `%ProgramFiles(x86)%`)
    *   Installations via the Python Launcher (`py.exe`).
...
*   **Interactive Interface**: Presents a clear, numbered list of found Python installations to choose from.
...
*   **User-Friendly Launcher**: Comes with a `.bat` file to handle the PowerShell execution policy, allowing you to run it with a simple double-click.
